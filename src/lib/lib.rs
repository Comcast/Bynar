...
impl ClientSocket {
    pub fn connect(host: &str, port: &str, server_publickey: &str) -> BynarResult<Self> {
        ...
        Ok(ClientSocket { socket: requester })
    }
    ...
    pub fn add_disk_request(
        ...
        let add_response = self.send_and_get(o)?;
    ...
    pub fn list_disks_request(&self) -> BynarResult<Vec<Disk>> {
        let mut o = Operation::new();
        debug!("Creating list operation request");
        o.set_Op_type(Op::List);

        let disks_response = self.send_and_get(o)?;
    ...
    pub fn safe_to_remove_request(&self, path: &Path) -> BynarResult<bool> {
        let mut o = Operation::new();
        debug!("Creating safe to remove operation request");
        o.set_Op_type(Op::SafeToRemove);
        o.set_disk(format!("{}", path.display()));

        let safe_response = self.send_and_get(o)?;
    ...
    pub fn remove_disk_request(
        ...
        let remove_response = self.send_and_get(o)?;
    ...
    fn send_and_get(&self, o: Operation) -> BynarResult<Vec<u8>> {
